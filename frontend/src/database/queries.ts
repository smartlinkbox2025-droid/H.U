...
import type {
  Property, Customer, Contract, Invoice, Payment, DocumentFile, ActivityLog,
} from '../models/types';
import { addMonths, differenceInCalendarMonths, addDays } from 'date-fns';
...
export function calculateIntervals(
  ...
  const monthsPerCycle = frequency === 'monthly' ? 1 : frequency === 'quarterly' ? 3 : 12;
...
async function nextInvoiceNumber(): Promise<string> {
  ...
  const yearOnes = all
    ...
    .filter((n) => !isNaN(n));
  const max = yearOnes.length ? Math.max(...yearOnes) : 0;
...
export async function createContractWithInvoices(
  ...
  await db.transaction(
    ...
    async () => {
      ...
      // Generate invoice numbers sequentially using a fresh scan each time
      const year = new Date().getFullYear();
      const prefix = `INV-${year}-`;
      ...
      await db.activityLogs.add({
        id: uuid(),
        action: 'إبرام عقد جديد',
        module: 'contracts',
        timestamp: new Date(),
        details: `تم إنشاء ${intervals.length} فاتورة`,
      });
    }
  );
...
export async function terminateContract(id: string): Promise<void> {
  ...
  await db.transaction('rw', db.contracts, db.properties, db.activityLogs, async () => {
    ...
    await db.properties.update(contract.propertyId, {
      status: 'vacant',
      updatedAt: new Date(),
    });
    await db.activityLogs.add({
...
export async function deleteContract(id: string): Promise<void> {
  ...
  await db.transaction(
    'rw',
    db.contracts, db.invoices, db.payments, db.properties, db.activityLogs,
    async () => {
      ...
      await db.properties.update(contract.propertyId, {
        status: 'vacant', updatedAt: new Date(),
      });
      await db.activityLogs.add({
        id: uuid(),
        action: 'حذف عقد وفواتيره',
        module: 'contracts',
        timestamp: new Date(),
        details: id,
      });
...
export async function recordPayment(
  ...
  const id = uuid();
  await db.transaction(
    'rw', db.payments, db.invoices, db.contracts, db.activityLogs,
    async () => {
...
export async function uploadDocument(
  file: File,
  relatedType: DocumentFile['relatedType'],
  relatedId: string
): Promise<void> {
...
export async function importBackup(json: string): Promise<{ ok: boolean; message: string }> {
  ...
  catch { return { ok: false, message: 'الملف تالف أو غير صالح' }; }
      ...
      const revive = <T extends Record<string, any>>(arr: T[], dateFields: string[]) =>
        (arr || []).map((r) => {
          ...
          for (const f of dateFields) {
            if (clone[f]) clone[f] = new Date(clone[f]);
          }
          return clone as T;
        });
