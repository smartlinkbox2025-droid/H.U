...
import {
  createContractWithInvoices, terminateContract, deleteContract,
...
import { Plus, FileText, Trash2, XCircle, Eye } from 'lucide-react';
...
export default function Contracts() {
  ...
  const printContract = async (c: Contract) => {
    ...
    await generateArabicPDF({
      title: 'عقد',
      subtitle: `${c.contractType === 'rent' ? 'إيجار' : 'بيع'} · ${fmtDate(c.createdAt)}`,
      sections: [
        {
          heading: 'أطراف العقد',
          paragraphs: [
            `الطرف الأول: ${propName(c.propertyId)}`,
            `الطرف الثاني: ${custName(c.customerId)}`,
          ],
        },
        {
          ...
          table: {
            ...
            rows: [
              [AR.contract.type, c.contractType === 'rent' ? 'إيجار' : 'بيع'],
        ...
        },
        {
          ...
          table: {
            ...
            rows: relatedInvoices.map((i) => [
              i.invoiceNumber, fmtDate(i.dueDate), fmtMoney(i.amountDue),
              AR.invoice.statuses[i.status],
            ]),
          },
        },
      ],
      filename: `عقد_${c.id}.pdf`,
    });
  };
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-2xl font-bold flex items-center gap-2">
            <FileText className="h-6 w-6" /> {AR.nav.contracts}
          </h2>
          <p className="text-sm text-muted-foreground mt-1">
            إبرام العقود مع توليد الأقساط والفواتير آلياً.
          </p>
        </div>
        <div className="flex gap-2">
          <Select value={statusFilter} onValueChange={(v) => setStatusFilter(v as any)}>
            ...
            <SelectContent>
              ...
              {(Object.keys(AR.contract.statuses) as ContractStatus[]).map((k) => (
                <SelectItem key={k} value={k}>{AR.contract.statuses[k]}</SelectItem>
              ))}
            </SelectContent>
          </Select>
                ...
                <TableRow key={c.id} data-testid={`contract-row-${c.id}`}>
                  ...
                  <TableCell>
                    <div className="flex items-center gap-1">
                      <Button size="icon" variant="ghost" onClick={() => setViewing(c)} data-testid={`view-contract-${c.id}`}>
                        <Eye className="h-4 w-4" />
                      </Button>
                      <Button size="icon" variant="ghost" onClick={() => printContract(c)} data-testid={`print-contract-${c.id}`}>
                        <FileText className="h-4 w-4" />
                      </Button>
                      {c.status === 'active' && (
                        <Button size="icon" variant="ghost" className="text-warning" onClick={() => onTerminate(c.id!)} data-testid={`terminate-contract-${c.id}`}>
                          <XCircle className="h-4 w-4" />
                        </Button>
                      )}
                      <Button size="icon" variant="ghost" className="text-destructive" onClick={() => onDelete(c.id!)} data-testid={`delete-contract-${c.id}`}>
...
function ContractDialog({
  open, onOpenChange, properties, customers,
}: {
  open: boolean;
  ...
  const submit = async () => {
    if (!form.propertyId || !form.customerId || !form.totalAmount) {
      toast.error('يرجى اختيار العقار والعميل وإدخال القيمة');
      return;
    }
    try {
      const { invoicesCreated } = await createContractWithInvoices({
      ...
      toast.success(`تم إبرام العقد وتوليد ${invoicesCreated} فاتورة تلقائياً`);
      onOpenChange(false);
    } catch (e: any) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl" data-testid="contract-dialog">
        <DialogHeader>
          <DialogTitle>{AR.contract.addNew}</DialogTitle>
        </DialogHeader>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <Field label={AR.contract.customer}>
            <Select value={form.customerId} onValueChange={(v) => setForm({ ...form, customerId: v })}>
              ...
              <SelectContent>
                {customers.length === 0 && <div className="px-3 py-2 text-xs text-muted-foreground">لا يوجد عملاء — أضف عميلاً أولاً</div>}
          ...
          <Field label={AR.contract.paymentFrequency}>
            <Select value={form.paymentFrequency} onValueChange={(v) => setForm({ ...form, paymentFrequency: v as PaymentFrequency })}>
              ...
              <SelectContent>
                {(Object.keys(AR.contract.frequencies) as PaymentFrequency[]).map((k) => (
                  <SelectItem key={k} value={k}>{AR.contract.frequencies[k]}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          </Field>
          <Field label={AR.contract.startDate}>
            <Input type="date" value={form.startDate} onChange={(e) => setForm({ ...form, startDate: e.target.value })} data-testid="contract-start-input" />
          </Field>
          <Field label={AR.contract.endDate}>
            <Input type="date" value={form.endDate} onChange={(e) => setForm({ ...form, endDate: e.target.value })} data-testid="contract-end-input" />
          </Field>
          <Field label={AR.contract.totalAmount}>
            <Input type="number" value={form.totalAmount} onChange={(e) => setForm({ ...form, totalAmount: parseFloat(e.target.value) })} data-testid="contract-amount-input" />
          </Field>
          <Field label={AR.contract.penaltyRate}>
            <Input type="number" step="0.1" value={form.penaltyRate} onChange={(e) => setForm({ ...form, penaltyRate: parseFloat(e.target.value) })} data-testid="contract-penalty-input" />
          </Field>
        </div>
...
function ContractView({
  contract, onClose, propertyName, customerName, invoices,
}: {
  contract: Contract;
  onClose: () => void;
  propertyName: string;
      ...
      <DialogContent className="max-w-3xl" data-testid="contract-view-dialog">
        ...
        <div className="space-y-4 text-sm">
          ...
          <div>
            ...
            <div className="max-h-60 overflow-auto rounded-2xl border">
              <Table>
                <TableHeader>
                  <TableRow>
