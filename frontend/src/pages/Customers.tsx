      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-2xl font-bold flex items-center gap-2">
            <Users className="h-6 w-6" /> {AR.nav.customers}
          </h2>
                ...
                <TableRow key={c.id} data-testid={`customer-row-${c.id}`}>
                  ...
                  <TableCell>
                    <div className="flex items-center gap-1">
                      <Button size="icon" variant="ghost" onClick={() => { setEditing(c); setDialogOpen(true); }} data-testid={`edit-customer-${c.id}`}>
                        <Pencil className="h-4 w-4" />
                      </Button>
                      <Button size="icon" variant="ghost" className="text-destructive" onClick={() => onDelete(c.id!, c.fullName)} data-testid={`delete-customer-${c.id}`}>
...
}: { open: boolean; onOpenChange: (v: boolean) => void; editing: Customer | null }) {
    ...
    try {
      if (editing?.id) {
        ...
        await createCustomer({
          fullName: form.fullName!, nationalId: form.nationalId!, phone: form.phone!,
          email: form.email || '', address: form.address || '', city: form.city || 'الرياض',
          notes: form.notes,
        });
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl" data-testid="customer-dialog">
        <DialogHeader>
          <DialogTitle>{editing ? AR.customer.editTitle : AR.customer.addNew}</DialogTitle>
        </DialogHeader>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <Field label={AR.customer.fullName} className="md:col-span-2">
            <Input value={form.fullName || ''} onChange={(e) => setForm({ ...form, fullName: e.target.value })} data-testid="customer-name-input" />
          </Field>
          <Field label={AR.customer.nationalId}>
            <Input value={form.nationalId || ''} onChange={(e) => setForm({ ...form, nationalId: e.target.value })} data-testid="customer-id-input" />
          </Field>
          <Field label={AR.customer.phone}>
            <Input dir="ltr" value={form.phone || ''} onChange={(e) => setForm({ ...form, phone: e.target.value })} data-testid="customer-phone-input" />
          </Field>
          <Field label={AR.customer.email}>
            <Input type="email" dir="ltr" value={form.email || ''} onChange={(e) => setForm({ ...form, email: e.target.value })} data-testid="customer-email-input" />
          </Field>
          ...
          <Field label={AR.customer.address} className="md:col-span-2">
            <Input value={form.address || ''} onChange={(e) => setForm({ ...form, address: e.target.value })} data-testid="customer-address-input" />
          </Field>
          <Field label={AR.customer.notes} className="md:col-span-2">
