        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3">
          <div className="lg:col-span-2 relative">
            ...
            <Input
              ...
              placeholder={`${AR.actions.search} بالاسم أو العنوان…`}
              value={search}
          ...
          <Select value={typeFilter} onValueChange={(v) => setTypeFilter(v as any)}>
            ...
            <SelectContent>
              ...
              {(Object.keys(AR.property.types) as PropertyType[]).map((k) => (
                <SelectItem key={k} value={k}>{AR.property.types[k]}</SelectItem>
              ))}
            </SelectContent>
          </Select>
          <Select value={statusFilter} onValueChange={(v) => setStatusFilter(v as any)}>
            ...
            <SelectContent>
              <SelectItem value="all">{AR.actions.all}</SelectItem>
              {(Object.keys(AR.property.statuses) as PropertyStatus[]).map((k) => (
                <SelectItem key={k} value={k}>{AR.property.statuses[k]}</SelectItem>
              ))}
            </SelectContent>
          </Select>
          <div className="grid grid-cols-2 gap-2">
            <Input
              ...
              type="number"
              placeholder={AR.property.minPrice}
              value={minPrice}
            ...
            <Input
              ...
              type="number"
              placeholder={AR.property.maxPrice}
              value={maxPrice}
...
function PropertyDialog({
  ...
  useMemo(() => {
    if (editing) setForm(editing);
      ...
      name: '', type: 'apartment', status: 'vacant',
      address: '', city: 'الرياض', price: 0, currency: 'SAR', notes: '',
    });
  }, [editing, open]);

  const submit = async () => {
    if (!form.name || !form.address || !form.city || !form.price) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl" data-testid="property-dialog">
        <DialogHeader>
          <DialogTitle>{editing ? AR.property.editTitle : AR.property.addNew}</DialogTitle>
        </DialogHeader>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <Field label={AR.property.name}>
            <Input
              value={form.name || ''}
          ...
          <Field label={AR.property.city}>
            <Select value={form.city || 'الرياض'} onValueChange={(v) => setForm({ ...form, city: v })}>
              ...
              <SelectContent>
                {CITIES_SA.map((c) => <SelectItem key={c} value={c}>{c}</SelectItem>)}
              </SelectContent>
            </Select>
          </Field>
          <Field label={AR.property.address} className="md:col-span-2">
            <Input
              value={form.address || ''}
          ...
          <Field label={AR.property.notes} className="md:col-span-2">
            <Textarea
              value={form.notes || ''}
              onChange={(e) => setForm({ ...form, notes: e.target.value })}
              data-testid="property-notes-textarea"
            />
          </Field>
        </div>
        <DialogFooter className="gap-2 sm:gap-2">
          <Button variant="outline" onClick={() => onOpenChange(false)} data-testid="property-cancel-button">
            {AR.actions.cancel}
          </Button>
          <Button onClick={submit} data-testid="property-save-button">
            {AR.actions.save}
          </Button>
        </DialogFooter>
      </DialogContent>
