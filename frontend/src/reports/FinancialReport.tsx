...
export default function FinancialReport() {
  ...
  const exportExcel = () => {
    exportToExcel({
      ...
      headers: [AR.payment.paymentDate, AR.payment.amountPaid, AR.payment.paymentMethod, AR.payment.referenceNumber],
      rows: filteredPayments.map((p) => [
        p.paymentDate, p.amountPaid, AR.payment.methods[p.paymentMethod], p.referenceNumber || '',
      ]),
      columnWidths: [16, 14, 14, 18],
    });
    ...
    <div className="space-y-4" data-testid="financial-report-page">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          تحليل مالي مفصل قابل للتصدير — الإيرادات والاستحقاقات خلال الفترة.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <Card className="glass border-0">
          <CardContent className="pt-6">
            <div className="text-xs text-muted-foreground">{AR.reports.revenue}</div>
            <div className="text-2xl font-bold num text-success mt-1">{fmtMoney(totalRevenue)}</div>
          </CardContent>
        </Card>
        <Card className="glass border-0">
          <CardContent className="pt-6">
            <div className="text-xs text-muted-foreground">إجمالي المستحقات</div>
            <div className="text-2xl font-bold num text-accent mt-1">{fmtMoney(totalDue)}</div>
          </CardContent>
        </Card>
        <Card className="glass border-0">
          <CardContent className="pt-6">
            <div className="text-xs text-muted-foreground">المتبقي غير المسدد</div>
            <div className="text-2xl font-bold num text-warning mt-1">{fmtMoney(totalOutstanding)}</div>
          </CardContent>
        </Card>
      </div>
